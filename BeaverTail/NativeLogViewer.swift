...
struct NativeLogViewer: NSViewRepresentable {
    ...
    let isMinimapActiveDrive: Bool // NEW PARAMETER FLAG
    var onLineIndexSelected: ((Int) -> Void)? = nil

    // Initializer for the Top Pane (Full Unfiltered Log View)
    init(lines: [String], textColor: NSColor, rules: [HighlightRule], selectedFraction: CGFloat?, directScrollNotificationName: Notification.Name?, isMinimapActiveDrive: Bool, onLineIndexSelected: @escaping (Int) -> Void) {
        self.lines = lines
        ...
        self.isMinimapActiveDrive = isMinimapActiveDrive // Bind flag state
        self.onLineIndexSelected = onLineIndexSelected
    }

    // Initializer for the Bottom Pane (Filtered Log View)
    init(filteredLines: [LogLine], textColor: NSColor, rules: [HighlightRule], selectedFraction: CGFloat?, onLineIndexSelected: @escaping (Int) -> Void) {
        self.lines = []
        ...
        self.isMinimapActiveDrive = false
        self.onLineIndexSelected = onLineIndexSelected
    }
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        // Listen for direct jump triggers from the bottom pane
    ...
    func updateNSView(_ nsView: NSScrollView, context: Context) {
        ...
        // CRUCIAL CHANGE: Only auto-scroll the top pane if the user is actively dragging the minimap!
        ...
        func tableViewSelectionDidChange(_ notification: Notification) {
            ...
            if selectedRow >= 0 {
                if let filterData = filteredLines {
                    ...
                    // For the top pane, the raw row index is already the correct file index!
