...
//  BeaverTail
//
//  Service layer: turns a user-entered Filter / highlight pattern into a compiled
//  `LineMatcher` and tests individual lines against it. This "regex compilation +
//  per-line matching" logic previously lived in the LogContent model file; it is
//  now a dedicated, UI-free service so the model owns only the memory-mapped byte
//  scanning that consumes the compiled matcher. Pure and off the main actor, so it
//  is independently unit-testable.
//

import Foundation

/// Compiles filter/highlight patterns into `LineMatcher`s and matches lines
/// against them. The heavy memory-mapped, parallel byte scanning stays in
/// `LogContent`, which consumes the `LineMatcher` produced here and calls
/// `matches(_:using:)` for its string-level fallback checks.
enum FilteringEngine {
    /// Compiles `pattern` into the fastest matcher kind it qualifies for (plain
    /// literal, literal alternation, or regex with a required-literal pre-filter),
    /// or `nil` for an empty / invalid-regex pattern.
    nonisolated static func compile(pattern: String, caseInsensitive: Bool) -> LineMatcher? {
        LineMatcher.make(pattern: pattern, caseInsensitive: caseInsensitive)
    }

    /// The required-literal pre-filters (one per top-level alternation branch), or
    /// `nil` when no branch yields a guaranteed literal. Exposed for callers/tests
    /// that reason about pre-filtering.
    nonisolated static func requiredLiterals(in pattern: String) -> [String]? {
        LineMatcher.requiredLiterals(in: pattern)
    }

    /// The longest literal an alternation-free branch must contain, or `nil`.
    nonisolated static func requiredLiteral(in pattern: String) -> String? {
        LineMatcher.requiredLiteral(in: pattern)
    }

    /// Pure per-line test: does `line` match `matcher`? Allocation-light; uses
    /// `memmem` / a case-folding byte scan for literals and `NSRegularExpression`
    /// only for true regex matchers.
    nonisolated static func matches(_ line: String, using matcher: LineMatcher) -> Bool {
...
@inline(__always)
    nonisolated func asciiInsensitiveContainsPtr(_ hay: UnsafePointer<UInt8>, _ len: Int, _ needle: UnsafePointer<UInt8>?, _ nlen: Int) -> Bool {
